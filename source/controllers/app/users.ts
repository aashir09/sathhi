...
import { head } from '../../routes/sampleRoutes';
...
const addUserAuthData = async (body: any) => {
    ...
    try {
        ...
        if (checkResult && checkResult.length > 0) {
            ...
            result = await header.query(sql, [new Date(new Date().toUTCString())]);
                            ...
                            if (result && result.affectedRows > 0) {
                                ...
                                if (userResult && userResult.length > 0) {
                                    for (let i = 0; i < userResult.length; i++) {
                                        let userDetailResponse: any = await controller.getUserData(userResult[i]);
                                        userResult[i] = { ...userResult[i], ...userDetailResponse };
                                    }
                                }
                            ...
                            if (result && result.affectedRows > 0) {
                                ...
                                if (signJWTResult && signJWTResult.token) {
                                    ...
                                    if (insertRefTokenResult && insertRefTokenResult.affectedRows > 0) {
                                        ...
                                        let _customFieldDataResult: any = await controller.getCustomFieldData(userResult[0].id);
...
const getMasterData = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (isCustomFieldEnabled) {
            ...
            result14 = await header.query(sql14);
            if (result14 && result14.length > 0) {

                for (let i = 0; i < result14.length; i++) {
                    if (result14[i].valueList) {
                        const valueListArray: string[] = result14[i].valueList.includes(';') ? result14[i].valueList.split(";") : [result14[i].valueList];
                        result14[i].valueList = valueListArray;
                    }
                    if (result14[i].valueTypeId == 10 && result14[i].defaultValue) {
                        const valueListArray: string[] = result14[i].defaultValue.includes(';') ? result14[i].defaultValue.split(";") : [result14[i].defaultValue];
                ...
                if (result && result.length > 0) {
                    ...
                    if (userId) {
                        if (userId != req.body.id) {
                            ...
                            let getViewProfileHistoryResult = await header.query(getViewProfileHistorySql);
                            ...
                            if (updateAddressResult && updateAddressResult.affectedRows > 0) {
                                ...
                                if (result && result.affectedRows > 0) {
                                    ...
                                    if (isCustomFieldEnabled && req.body.customFields != null && req.body.customFields.length > 0) {
                                        ...
                                        let userpersonaldetailcustomdataResult = await header.query(userpersonaldetailcustomdataSql);
                                        ...
                                        if (userpersonaldetailcustomdataResult && userpersonaldetailcustomdataResult.length > 0) {
                                            ...
                                            let customUpdateResult = await header.query(customUpdateSql);
                                            ...
                                            let customAddResult = await header.query(customAddSql);
                                    ...
                                    if (result && result.length > 0) {
                                        ...
                                        let _customFieldDataResult: any = await controller.getCustomFieldData(req.body.id);
                                        if (_customFieldDataResult && _customFieldDataResult.length > 0) {
                                            // console.log(_customFieldDataResult);
...
async function makememberid(length: number) {
    let result = '';
    let format: string = ''
    let formatSql = await header.query(`SELECT value FROM systemflags WHERE name = 'memberIdFormat'`);
    ...
    if (format == 'Only Numeric') {
        ...
        let lettersSql = await header.query(`SELECT value FROM systemflags WHERE name = 'prefixLetters'`);
        ...
        let lettersSql = await header.query(`SELECT value FROM systemflags WHERE name = 'postfixLetters'`);
        let letters = lettersSql[0].value;
        const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
        const charactersLength = characters.length;
        let counter = 0;
...
const generateMemberIdTestingAPI = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
const searchUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        let genderVisibility = await header.query(`SELECT value FROM systemflags WHERE name = 'genderVisibility'`);
                ...
                if (distanceArray && distanceArray.length > 0) {
                    ...
                    if (getResult) {
                        for (let i = 0; i < getResult.length; i++) {
                            ...
                            // region to get user personal custom data
                            let _customFieldDataResult: any = await controller.getCustomFieldData(getResult[i].id);
                            if (_customFieldDataResult && _customFieldDataResult.length > 0) {
                                // console.log(_customFieldDataResult);
...
const getUsers = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (req.body.sortingby == 'nearestApplicant') {
            ...
            if (checkResult && checkResult.length >= 0) {
                ...
                for (let i = 0; i < checkResult.length; i++) {
                    let km = await distance((userResult && userResult.length > 0 && userResult[0].latitude) ? userResult[0].latitude : 21.144539, (userResult && userResult.length > 0 && userResult[0].longitude) ? userResult[0].longitude : 73.094200
                        , checkResult[i].latitude ? checkResult[i].latitude : 21.144539, checkResult[i].longitude ? checkResult[i].longitude : 73.094200, "K");
                    let distanceObj = {
        ...
        if (result) {
            let isVerifyProfile = await header.query(`SELECT value FROM systemflags WHERE name = 'isUserProfilePicApprove'`);
            for (let i = 0; i < result.length; i++) {
                // result[i].isVerifyProfilePic = true;
            ...
            let successResult = new ResultSuccess(200, true, 'Get Users Successfully', result, result.length, authorizationResult ? authorizationResult.token : '');
            return res.status(200).send(successResult);
        } else {
            let errorResult = new ResultError(400, true, 'Data Not Available', new Error('Data Not Available'), '');
            next(errorResult);
...
const completeUserProfileV2 = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Updating Users');
        ...
        if (authorizationResult.statusCode == 200) {
            ...
            checkDetailResult = await header.query(checkDetailSql);
            ...
            let screenDetailSql = await header.query(`SELECT * FROM registrationscreens WHERE screenDisplayNo = ` + screenNumber);
            ...
            switch (screenNumber) {
                ...
                case 1: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen1RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {

                        let updateUserSql = `UPDATE users SET gender = '` + req.body.gender + `' WHERE id = ` + userId;
                        let updateUserResult = await header.query(updateUserSql);
                ...
                case 2: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen2RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                        ...
                        let checkMail = await header.query(`SELECT * FROM users WHERE email = '` + req.body.email + `' AND id != ` + userId + ``)
                        if (checkMail && checkMail.length > 0) {
                            let errorResult = new ResultError(203, true, "Email Already Exist", new Error("Email Already Exist"), '');
                            next(errorResult);
                        } else {
                ...
                case 3: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen3RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                ...
                case 4: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen4RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                ...
                case 5: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen5RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                        ...
                        if (req.body.fatherDetails) {
                            ...
                            if (fatherDetail.id) {
                                ...
                                let updateResult = await header.query(updatetSql);
                            } else {
                                ...
                                let isertresult = await header.query(insertsql);
                ...
                case 6: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen6RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                        ...
                        if (checkDetailResult[0].addressId) {
                            ...
                            addressResult = await header.query(updatePermanentAddress);
                        } else {
                            ...
                            addressResult = await header.query(insertPermanentAddress);
                        ...
                        if (checkDetailResult[0].currentAddressId) {
                            ...
                            currentAddressResult = await header.query(updateCurrentAddress);
                        } else {
                            ...
                            currentAddressResult = await header.query(insertCurrentAddress);
                ...
                case 7: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen7RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                ...
                case 8: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen8RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {

                        let astrologicDetailSql = await header.query(`SELECT * FROM userastrologicdetail WHERE userId = ` + userId + ``);
                ...
                case 9: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen9RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                ...
                case 10: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen11RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                        ...
                        let preferencesDetail = await header.query(`SELECT * FROM userpartnerpreferences WHERE userId = ` + userId + ``);
                ...
                case 11: {
                    ...
                    let validationResult = header.validateRequiredFields(req, screen11RequiredFields);
                    if (validationResult && validationResult.statusCode == 200) {
                        let docCheck = await header.query(`SELECT * FROM userdocument WHERE userId = ` + userId + ``)
            ...
            if (result && result.affectedRows > 0) {
                ...
                let screenCount = await header.query(`SELECT COUNT(id) as count FROM registrationscreens WHERE isDisable = 0`)
                ...
                if (user[0].lastCompletedScreen < screenNumber) {
                    ...
                    let updateScreenResult = await header.query(updateScreenSql);
                }
                ...
                if (isCustomFieldEnabled && req.body.customFields != null && req.body.customFields.length > 0) {
                    ...
                    let checkCustomData = await header.query(`SELECT * FROM userpersonaldetailcustomdata WHERE userId = ` + userId + `  `);
                    ...
                    customResult = await header.query(customSql);
                    if (customResult && customResult.affectedRows > 0) {

                    }
                    else {
                        await header.rollback();
                        let errorResult = new ResultError(400, true, "users.updateUserProfileDetail() Error", new Error('Error While Inserting Custom Field Data'), '');
                        next(errorResult);
                    }
                }
                ...
                if (!flagError) {
                    let sql = `SELECT u.id, u.firstName,udd.fcmToken,u.stripeCustomerId, u.middleName, u.lastName, u.gender, u.email, u.contactNo, u.isVerifyProfilePic,u.lastCompletedScreen,u.isProfileCompleted,upd.isHideContactDetail
                                   ...
                                   LEFT JOIN educationtype edt ON edt.id = upd.educationTypeId
                                   WHERE ur.roleId = 2 AND u.id = ` + req.body.id + ``;
                    let responseResult = await header.query(sql);
                    console.log(sql)
                    if (responseResult && responseResult.length > 0) {
                        ...
                        let userflagvalues = `SELECT ufv.*, uf.flagName, uf.displayName FROM userflagvalues ufv
                                                    LEFT JOIN userflags uf ON uf.id = ufv.userFlagId
                                                    WHERE ufv.userId = ` + req.body.id;
                        responseResult[0].userFlags = await header.query(userflagvalues);
                                    ...
                                    if (adminUserResult[a].isReceiveMail) {
                                        let resultEmail = await sendEmail(config.emailMatrimonyNewUserRegister.fromName + ' <' + config.emailMatrimonyNewUserRegister.fromEmail + '>'
                                            ...
                                            , config.emailMatrimonyNewUserRegister.html
                                                ...
                                                .replace("[User's Email Address]", responseResult[0].email)
                                            , null, null);
                                        console.log(resultEmail)
                                    }
                                    if (adminUserResult[a].isReceiveNotification) {
                        ...
                        for (let i = 0; i < responseResult.length; i++) {
                            let userDetailResponse: any = await controller.getUserData(responseResult[i]);
                            responseResult[i] = { ...responseResult[i], ...userDetailResponse };
                        }
                        ...
                        await header.commit();
                        let successResult = new ResultSuccess(200, true, status + 'User Personal Detail', responseResult, 1, authorizationResult.token);
                        return res.status(200).send(successResult);

                    } else {
                        await header.rollback();
                        let errorResult = new ResultError(400, true, "users.completeUserProfileV2() Error", new Error('Error While Updating Data'), '');
                        next(errorResult);
                    }
                }
...
const TestingAPI = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        let successResult = new ResultSuccess(200, true, 'Get Master Data Successfully', data, 1, '');
        return res.status(200).send(successResult);
    } catch (error: any) {
        let errorResult = new ResultError(500, true, 'users.getUsers() Exception', error, '');
        next(errorResult);
