        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (proposalRequestSqlResult && proposalRequestSqlResult.length > 0) {
                    ...
                    let checkSql = await header.query(`SELECT * FROM userproposals WHERE userId = ` + req.body.proposalUserId + ` AND proposalUserId =` + userId + ``);
    ...
    try {
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.affectedRows > 0) {
                    let proposal = await header.query(`SELECT * FROM userproposals WHERE userId = ` + req.body.proposalUserId + ` AND proposalUserId = ` + userId);
                    let title;
                    let description
                    let fcmToken = "";
                    let dataBody = {
...
const cancelProposalRequest = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Canceling Proposals');
        let requiredFields = ['proposalUserId'];
        let validationResult = header.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            let authorizationResult = await header.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                ...
                if (result && result.affectedRows > 0) {
                    ...
                    let successResult = new ResultSuccess(200, true, 'Cancel User Proposals', result, 1, authorizationResult.token);
                    return res.status(200).send(successResult);
                } else {
                    let errorResult = new ResultError(400, true, "proposals.cancelProposals() Error", new Error('Error While Updating Data'), '');
                    next(errorResult);
                }
        ...
        let errorResult = new ResultError(500, true, 'proposals.cancelProposals() Exception', error, '');
        next(errorResult);
    }
};

const removeSentProposalRequest = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Remove Sent Proposals');
        let requiredFields = ['proposalUserId'];
        let validationResult = header.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            let authorizationResult = await header.validateAuthorization(req, res, next);
            if (authorizationResult.statusCode == 200) {
                ...
                let idSql = await header.query(`SELECT id FROM userproposals WHERE proposalUserId = ` + req.body.proposalUserId + ` AND userId =` + userId + ``)
                ...
                if (result && result.affectedRows > 0) {
                    let successResult = new ResultSuccess(200, true, 'Remove User Sent Proposal Successfully.', result, 1, authorizationResult.token);
                    return res.status(200).send(successResult);
                } else {
                    let errorResult = new ResultError(400, true, "proposals.removeSentProposalRequest() Error", new Error('Error While Removing Data'), '');
                    next(errorResult);
                }
...
export default { getProposalsGotByMe, getProposalsSendByMe, sendProposals, acceptRejectProposals, cancelProposalRequest, removeSentProposalRequest };
