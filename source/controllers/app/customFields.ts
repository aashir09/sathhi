...
const getCustomFields = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            // }
            let customFieldsResult = await header.query(customFieldsSql);
            ...
            if (customFieldsResult && customFieldsResult.length > 0) {
                ...
                for (let i = 0; i < customFieldsResult.length; i++) {
                    if (customFieldsResult[i].valueList) {
                        const valueArray: string[] = customFieldsResult[i].valueList.split(";");
                        customFieldsResult[i].valueList = valueArray;
                    }

                }
                let totalCount = countResult[0].totalRecords;
                let successResult = new ResultSuccess(200, true, 'Get Countries Successfully', customFieldsResult, totalCount, '');
...
const getCustomFieldsInResponse = async (req: Request, res: Response, next: NextFunction) => {
    try {
        logging.info(NAMESPACE, 'Getting CustomFields');
        let requiredFields = [''];
        let validationResult = header.validateRequiredFields(req, requiredFields);
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (userCustomDataResult && userCustomDataResult.length > 0) {
                ...
                let totalCount = countResult[0].totalRecords;
                let successResult = new ResultSuccess(200, true, 'Get Countries Successfully', userCustomDataResult, totalCount, '');
                return res.status(200).send(successResult);
            } else {
                let successResult = new ResultSuccess(200, true, 'Countries Not Available', [], 0, '');
                return res.status(200).send(successResult);
...
const isCustomFieldEnable = async () => {
    let result;
    try {
        let systemFlags = `SELECT * FROM systemflags where id = 45 and value = 1`;
        let systemFlagsResult = await header.query(systemFlags);
...
const getCustomFieldData = async (userId: String) => {
    let result = [];
    try {
        logging.info(NAMESPACE, 'Getting CustomFields');
        const isCustomFieldEnabled = await isCustomFieldEnable();
                ...
                if (customdata && customdata.length > 0) {
                    ...
                    for (let i = 0; i < customdata.length; i++) {
                        ...
                        if (customdata[i].valueTypeId == 10 && customdata[i].defaultValue) {
                            const valueListArray: string[] = customdata[i].defaultValue.includes(';') ? customdata[i].defaultValue.split(";") : [customdata[i].defaultValue];
...
const getUserData = async (user: any) => {
    ...
    try {
        ...
        if (user.pCountryLivingInId == 0) {
            ...
            pCountries = await header.query(`SELECT name FROM countries WHERE  id IN (` + user.pCountryLivingInId + `)`);
            pCountries = pCountries ? pCountries.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pReligionId == 0) {
            ...
            pReligions = await header.query(`SELECT name FROM religion WHERE  id IN (` + user.pReligionId + `)`);
            pReligions = pReligions ? pReligions.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pCommunityId == 0) {
            ...
            pCommunities = await header.query(`SELECT name FROM community WHERE  id IN (` + user.pCommunityId + `)`);
            pCommunities = pCommunities ? pCommunities.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pStateLivingInId == 0) {
            ...
            pStates = await header.query(`SELECT name FROM state WHERE  id IN (` + user.pStateLivingInId + `)`);
            pStates = pStates ? pStates.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pCityLivingInId == 0) {
            ...
            pCities = await header.query(`SELECT name FROM cities WHERE  id IN (` + user.pCityLivingInId + `)`);
            pCities = pCities ? pCities.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pEducationMediumId == 0) {
            ...
            pEducationMedium = await header.query(`SELECT name FROM educationmedium WHERE  id IN (` + user.pEducationMediumId + `)`);
            pEducationMedium = pEducationMedium ? pEducationMedium.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pEducationTypeId == 0) {
            ...
            pEducationType = await header.query(`SELECT name FROM educationtype WHERE  id IN (` + user.pEducationTypeId + `)`);
            pEducationType = pEducationType ? pEducationType.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pOccupationId == 0) {
            ...
            pOccupation = await header.query(`SELECT name FROM occupation WHERE  id IN (` + user.pOccupationId + `)`);
            pOccupation = pOccupation ? pOccupation.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pEmploymentTypeId == 0) {
            ...
            pEmploymentType = await header.query(`SELECT name FROM employmenttype WHERE  id IN (` + user.pEmploymentTypeId + `)`);
            pEmploymentType = pEmploymentType ? pEmploymentType.map((type: any) => type.name).join(", ") : '';
        }
        ...
        if (user.pAnnualIncomeId == 0) {
            ...
            pAnnualIncome = await header.query(`SELECT value FROM annualincome WHERE id IN (` + user.pAnnualIncomeId + `)`);
            pAnnualIncome = pAnnualIncome ? pAnnualIncome.map((type: any) => type.value).join(", ") : '';
        }
        ...
        if (user.pMaritalStatusId) {
            const matchingMaritalStatuses = rawData.maritalStatus.filter((c: any) => user.pMaritalStatusId.includes(c.id));
            pMaritalStatus = matchingMaritalStatuses.map((status: any) => status.name).join(", ");
        }

        let maritalStatus;
        if (user.maritalStatusId) {
            const matchingMaritalStatuses = rawData.maritalStatus.filter((c: any) => user.maritalStatusId == c.id);
            maritalStatus = matchingMaritalStatuses.map((status: any) => status.name).join(", ");
        }

        let pDiet;
        if (user.pDietId) {
            const diet = rawData.dietTypeList.filter((c: any) => user.pDietId.includes(c.id));
            pDiet = diet.map((status: any) => status.name).join(", ");
        }

        let diet;
        if (user.dietId) {
            const filterdiet = rawData.dietTypeList.filter((c: any) => user.dietId == c.id);
            diet = filterdiet.map((status: any) => status.name).join(", ");
        }
...
export default { getCustomFields, getCustomFieldsInResponse, isCustomFieldEnable, getCustomFieldData, getUserData };