    ...
    try {
      if (searchInput) {
        ...
        const res = await APIservice.httpPost(
          '/api/admin/report/getPremiumAppUser',
          obj,
          token,
          refreshToken
        );
  ...
  const handlePrint = async () => {
    ...
    for (let i = 0; i < res.recordList.length; i++) {
      html +=
        ...
        ` </td>
         <td class="td4">` +
        res.recordList[i].packageName +
        '-' +
        res.recordList[i].value +
        ' months' +
        ` </td>
         <td class="td5">₹` +
        (Math.round(res.recordList[i].netAmount * 100) / 100).toFixed(2) +
        ` </td>
         <td class="td6">` +
        format(new Date(res.recordList[i].startDate), dateData) +
        ` </td>
         <td class="td7">` +
        format(new Date(res.recordList[i].endDate), dateData) +
        ` </td>
         </tr>`;
    }
    html += `</tbody>
