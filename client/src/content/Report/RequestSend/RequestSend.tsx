...
const RequestSend = () => {
  ...
  const getdata = async (startIndex: number, fetchRecord: number) => {
    try {
      ...
      const res = await APIservice.httpPost(
        '/api/admin/report/getTopProposalSendReqReport',
        obj,
        token,
        refreshToken
      );
  ...
  return (
    <div>
      ...
      />
      <Helmet>
        <title>Top Requests Send</title>
      </Helmet>
      <PageTitleWrapper>
        <Box pt={1.3} pb={1.3} pl={1}>
          <Grid
            container
            ...
            <Grid item>
              ...
              <Tooltip title="Export File" arrow>
                ...
                >
                  <CSVLink
                    data={data1}
                    filename={'RequestSendReport.csv'}
                    style={{
                      '&:hover': { background: theme.colors.primary.lighter },
                      color: theme.palette.primary.main
                    }}
