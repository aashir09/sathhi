...
function Religion() {
  ...
  const [isReligionError, setReligionError] = useState(false);
  const [ReligionErrorMsg, setReligionErrorMsg] = useState('');
  ...
  const handleSwitchCheck = async () => {
    ...
    const res = await APIservice.httpPost(
      '/api/admin/religion/activeInactiveReligion',
      obj,
      token,
      refreshToken
    );
    ...
    if (flag) {
      try {
        if (v1.id) {
          ...
          const res = await APIservice.httpPost(
            '/api/admin/religion/insertUpdateReligion',
            val,
            token,
            refreshToken
          );
          if (res && res.status == 200) {
            ...
            setReligionErrorMsg('Religion already exists!');
          }
        } else {
          const token = localStorage.getItem('SessionToken');
          const refreshToken = localStorage.getItem('RefreshToken');
                        ...
                        <>
                          <TableContainer className="religiontableContainer">
                            <Table stickyHeader>
                              ...
                              <TableBody>
                                {Religion.map((arr: any, index: number) => {
                                  return (
                      ...
                      <div>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {isReligionError && ReligionErrorMsg}
                            </FormHelperText>
                            <Typography>
                              ...
                              <Button disabled={credentail?.email === "demo@admin.com"} onClick={saveReligion} variant="outlined" style={{marginRight: '10px'}}>Save</Button>
                            </Typography>
