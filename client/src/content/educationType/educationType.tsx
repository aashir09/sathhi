...
function EducationType() {
  ...
  const [educationType, setEducationType] = React.useState<any>(initialState);
  const [isNameError, setNameError] = useState(false);
  const [NameErrorMsg, setNameErrorMsg] = useState('');
  const [isEducationTypeError, setEducationTypeError] = useState(false);
  const [EducationTypeErrorMsg, setEducationTypeErrorMsg] = useState('');
  ...
  const validateName = (arr: any) => {
    ...
    if (value) {
      ...
      setNameError(true);
      setNameErrorMsg('Education Type is required');
    }
  };
  ...
  const validateForm = (e: any) => {
    e.preventDefault();
    var flag = true;
    if (!educationType.name) {
      setNameError(true);
      setNameErrorMsg('Education Type is required');
      flag = false;
    } else {
      if (reg.test(educationType.name)) {
        setNameError(false);
        setNameErrorMsg('');
        flag = true;
      } else {
  ...
  const handleClickisAdd = async () => {
    setEducationType(initialState);
    setNameError(false);
    setNameErrorMsg('');
    setEducationTypeError(false);
    setEducationTypeErrorMsg('');
    setIsOpen(true);
  };
  ...
  const handleSwitchCheck = async () => {
    ...
    const res = await APIservice.httpPost(
      '/api/admin/education/activeInactiveEducationType',
      obj,
      token,
      refreshToken
    );
  ...
  const EducationDialog = (arr: any) => {
    const { name, value } = arr.target;
    setEducationType({ ...educationType, [name]: value });
    setIsOpen(true);
    setEducationTypeError(false);
    setEducationTypeErrorMsg('');
  };
    ...
    if (flag) {
      try {
        if (educationType.id) {
          const token = localStorage.getItem('SessionToken');
          const refreshToken = localStorage.getItem('RefreshToken');
          let val = { id: educationType.id, name: educationType.name };
          const res = await APIservice.httpPost(
          ...
          if (res && res.status == 200) {
            ...
            setEducationTypeErrorMsg('Education Type already exists!');
          } else if (res.status == 500) {
            setIsOpen(false);
            toast.error(res.message, {
              autoClose: 6000,
          ...
          const refreshToken = localStorage.getItem('RefreshToken');
          const res = await APIservice.httpPost(
            '/api/admin/education/insertUpdateEducationType',
            educationType,
            token,
            refreshToken
          );
                ...
                <Grid item>
                  {isWritePermission ? <>
                    <Button
                      ...
                      size="small"
                    >
                      <AddTwoToneIcon fontSize="small" /> Create Education Type
                    </Button>
                    <Button
                      className="button"
                      sx={{
            ...
            <>
                        ...
                        <>
                          <TableContainer className="educationtableContainer">
                            <Table stickyHeader>
                              ...
                              <TableBody>
                                {educationTypes.map((arr: any, index: number) => {
                                  return (
              ...
              <div>
                ...
                >
                  ...
                  >
                    {educationType.status === 0 ? 'Inactive' : 'Active'}
                  </DialogTitle>
                  <DialogContent>
                    <DialogContentText
                      style={{ fontSize: '1rem', letterSpacing: '0.00938em' }}
                    >
                      {educationType.status === 0
              ...
              <div>
                ...
                >
                  <BootstrapDialogTitle
                    ...
                    onClose={handleCloseEducationDailog}
                  >
                    {educationType.id ? 'Edit Education Type' : 'Add Education Type'}
                  </BootstrapDialogTitle>
                  ...
                  >
                    ...
                    >
                      {isEducationTypeError && EducationTypeErrorMsg}
                    </FormHelperText>
                    <Typography>
